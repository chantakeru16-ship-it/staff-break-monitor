...
if "off_shift"      not in st.session_state: st.session_state.off_shift      = set()  # staff marked as not working today
...
def get_shift_metrics(shift_name):
    ...
    shift_staff = staff_df[
        ...
        (~staff_df["Name"].isin(st.session_state.off_shift))  # exclude off-shift staff
    ]
...
def render_shift(shift_name):
    ...
    for _, row in shift_staff.iterrows():
        ...
        # On Shift / Off Shift radio toggle
        ...
        if shift_status == "Off Shift" and staff not in st.session_state.off_shift:
            ...
            # Remove from active breaks if marked off
        ...
        # Break In / Break Out buttons — only show if On Shift
        if not is_off:
            if on_break:
